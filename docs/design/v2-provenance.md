...
직접 실행한 `ssh`와 로컬 파일을 원격 목적지로 보내는 `scp` upload는 로컬 snapshot으로 원격 결과를 증명할 수 없다. 원격 mutation 가능성은 로컬 부작용 가능성과 별도로 판정한다. 성공한 원격 가능 명령은 `last_remote_mutation_seq`를 올리고 verification 필요 상태로 둔다. 별도 검증 invocation이 시작될 때 고정한 `covers.through_seq`가 해당 remote epoch 이상이어야 Stop을 허용한다. 같은 invocation 안의 원격 변경+검증은 스스로를 cover하지 않는다.
