...
- 직접 `ssh`와 로컬→원격 `scp`는 로컬 파일 변경 관측 대상이 아닌 `remote-only` epoch로 기록한다. 이후 별도 검증 invocation의 frozen `covers.through_seq`가 그 epoch 이상일 때만 해소한다. 로컬 redirect/pipeline/download/chain/substitution/제어 옵션이 섞인 명령은 이 완화 경로를 사용하지 않는다.
...
직접 실행한 `ssh`와 로컬 파일을 원격 목적지로 보내는 `scp` upload는 로컬 snapshot으로 원격 결과를 증명할 수 없다. 성공한 명령은 `last_remote_mutation_seq`를 올리고 verification 필요 상태로 둔다. 별도 검증 invocation이 시작될 때 고정한 `covers.through_seq`가 해당 remote epoch 이상이어야 Stop을 허용한다. 같은 invocation 안의 원격 변경+검증은 스스로를 cover하지 않는다.

이 완화는 원격 상태를 관측해 clean을 증명한다는 뜻이 아니다. 로컬 redirect, pipe, chain, command substitution, `scp` download, 해석하기 어려운 SSH control/log option이 있는 명령은 remote-only로 분류하지 않고 기존 local mutation-capable 보수 경로를 유지한다.
