...
# qwen-code 도구명 → core 게이트가 인식하는 정규명. core/contract.py의
# EDIT_TOOLS/SHELL_TOOLS는 Claude/codex 이름이라 qwen 이름을 그대로 넘기면
# family가 전부 "other"로 분류돼 게이트가 무력화된다(core 무수정 원칙 → 어댑터에서 변환).
QWEN_TOOL_ALIASES: dict[str, str] = {
...
def canonical_tool_name(payload: Mapping[str, JsonValue]) -> str:
    raw = _string(payload.get("tool_name"))
    return QWEN_TOOL_ALIASES.get(raw, raw)
...
def tool_success(payload: Mapping[str, JsonValue]) -> bool:
    ...
    # exit_code/success 신호가 전혀 없을 때 공유 텍스트 폴백 적용(codex_cli/common.py와 동일).
...
def canonical_invocation(
    ...
    agent = _string(payload.get("agent")) or "qwen"
