...
- Ledger migration is explicit: set `FABLE_LITE_AUTO_MIGRATION=1` in the hook process environment. Migration still requires the packaged W9/W10 receipts to be green. A v2 ledger with a legacy invocation missing `status` is never rewritten while this opt-in is off; it loads as attribution-degraded and destructive R2 operations fail closed. With the opt-in on, missing values are backfilled as `closed` through an immutable `ledger.v2-invocation-status.json.bak`, schema validation, and atomic replacement.
